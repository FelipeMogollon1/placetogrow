...
<template>
    <section>
        <header>
            <h2 class="text-lg font-medium text-gray-900">{{$t('user.update_password')}}</h2>

            <p class="mt-1 text-sm text-gray-600">
                {{$t('user.update_password_additional')}}
            </p>
        </header>

        <form @submit.prevent="updatePassword" class="mt-6 space-y-6">
            <div>
                <InputLabel for="current_password" :value="$t('user.current_password')" />
