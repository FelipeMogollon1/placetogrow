...
<template>
    <Head :title="$t('role.create_role')" />
    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ $t('role.create_role') }}</h2>
                <Link
                                ...
                                <div v-for="(permissions, group) in groupedPermissions" :key="group">
                                    ...
                                    <div class="ml-6">
                                        <div v-for="permission in permissions" :key="permission" class="flex items-center mt-2">
                                            ...
                                            />
                                            <InputLabel :for="`${group}.${permission}`" :value="$t(`role.permissions.${permission}`)" class="ml-2" />
                                        </div>
                                    </div>
                                </div>
                            </div>
